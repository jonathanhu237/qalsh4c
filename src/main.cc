...
int main(int argc, char** argv) {
    ...
    synthesize_dataset_command
        ->add_option("-t,--data-type", data_type, "Data type for the dataset (uint8, int, float, double)")
        ->default_val("double")
        ->check(CLI::IsMember({"uint8", "int", "float", "double"}));
    ...
    convert_dataset->add_option("-n,--dataset-name", dataset_name, "The name of the dataset (sift, gist, trevi, p53)")
        ->check(CLI::IsMember({"sift", "gist", "trevi", "p53"}))
        ->required();
    ...
    // ------------------------------
    // quadtree sampling estimate
    // ------------------------------
    CLI::App* quadtree_sampling =
        sampling->add_subcommand("quadtree", "Generate samples using the weights generated by Quadtree");
    quadtree_sampling->callback([&]() { weights_generator = std::make_unique<QuadtreeWeightsGenerator>(); });
